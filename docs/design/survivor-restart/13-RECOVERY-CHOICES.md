...
수정 커밋 `3651257`의 [최종 원격 검사](https://github.com/Seung-Won-Yu/rune-drift-survivors/actions/runs/34454623750)에서 코어 72개, 새 게임 브라우저 53개, 기존 게임 브라우저 86개로 총 211개를 통과했다. Pages production 검사와 공개 배포도 완료했다. [CI 기록](./recovery-release-ci-2026-09-10.json).

적 160마리의 12초 부하 검사에서 프레임 시간 p95는 1280×720에서 33.4ms, 390×844에서 16.7ms, 740×360에서 16.8ms였다. 50ms 초과 프레임은 데스크톱 1개, 나머지 0개로 검사 기준을 통과했다. 이는 CI 환경의 측정이며 모든 기기의 성능을 보장하지 않는다.

2026-09-10 17:36 KST에 공개 주소의 JS·CSS·preload·캐릭터와 적 이미지 5개의 해시가 로컬 Pages 빌드와 일치함을 확인했다. 개발용 QA 기능 없이 이동·일시정지·재개 후 첫 성장에 도달했고, 현재 체력 표시와 카드 선택 후 전투 재개를 확인했다. 390×844의 터치 조이스틱과 가로 넘침 없음, 페이지 오류와 HTTP 실패 없음도 확인했다. 인앱 브라우저의 공개 시작 화면도 정상 로드됐다. [공개 검증 기록](./recovery-public-release-2026-09-10.json).
