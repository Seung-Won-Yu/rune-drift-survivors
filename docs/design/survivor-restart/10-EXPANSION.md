...
GPU 가속을 끈 확장 부하(연소·둔화·넓은 궤도 포함)도 1280/390/740 세 화면 모두 통과했다. [CPU 기록](./expansion-cpu-2026-09-09.json). 원격 전체 검사와 공개 배포 확인도 완료했다. 아래 공개 전달 기록에서 실행 커밋과 결과를 확인할 수 있다. 기존 물리 모바일/iOS Safari 검증 한계는 유지한다.
...
최종 실행 커밋 `88a1d38`의 구현과 검증 범위를 대조했다. 전체 플레이 뒤의 변경은 요약 표시와 적 프레임 캐시이며 전투 규칙은 동일하다.

| 요구사항 | 구현 및 직접 확인 근거 | 상태 |
| --- | --- | --- |
| 무기 3종의 2갈래 선택 | `expansion.js`의 6개 전문화, `game.js`의 제시/상호 배제와 실제 공격, 코어 범위·피해·연소 검사, 18개 정상 경로 기록 | 확인 |
| 유물 6개의 서로 다른 효과 | `expansion.js`의 6개 유물, `game.js`의 점화·정예 피해·둔화·연사·회복·반격, 중복/치명 피해/재사용 검사 | 확인 |
| 제단과 상자의 위험·보상 | `encounters.js`의 위치·시간·수락/거절·진행·지급 상태, 실제 10초/18초 브라우저 검사, 세 전체 플레이의 완료 이벤트와 유물 2개 | 확인 |
| 선택을 이해하는 화면과 조작 | 카드의 손익, 일시정지/결과의 선택과 수치, 방향/진행 안내, 키보드 선택/거절 및 320/390/740 화면 검사와 캡처 | 확인 |
| 기록의 호환성과 새 판 초기화 | 프로필의 허용된 발견 ID·최근 판 선택, 이전 형식/중복 저장 검사, 전체 플레이의 결과 저장 및 재도전 초기화 | 확인 |
| 반복 플레이·안정성·성능 | 정상 18판·실제 시간 3판, 개체 상한, 연소/둔화 최대 부하, 원격 새 게임 97개 검사와 production 검증 통과 | 확인 |
| 문서·main·공개 배포 | README와 안내 갱신, main `88a1d38`, 원격 기존 게임 86개 검사 및 Pages 배포 성공, 공개 코드·스타일·이미지 해시와 브라우저 실행 확인 | 확인 |

실기기 성능과 사람의 재미 평가는 이 자동 검증으로 대체하지 않는다.

## 공개 전달 기록

실행 커밋 `88a1d38084f402626705b328284c2827ea4d299b`의 [GitHub Actions 실행](https://github.com/Seung-Won-Yu/rune-drift-survivors/actions/runs/34297063334)은 build/deploy 모두 성공했다. 코어 57개, 새 게임 브라우저 40개, 기존 3D 게임 회귀 86개와 production 검사를 통과한 뒤 Pages에 배포됐다. [CI 원본 상태와 집계](./expansion-release-ci-2026-09-09.json).

원격 확장 부하의 p95는 1280×720에서 33.4ms, 390×844에서 16.8ms, 740×360에서 16.7ms로 모두 기존 기준 50ms 미만이다. 각 12초 검사에서 50ms 초과 프레임은 각각 1/0/0개였다. 이전 실패와 달리 이번 실행은 캐시 적용 후 전체 게이트를 통과했으며 실기기 측정으로 해석하지 않는다.

2026-09-09 10:15 KST에 [공개 게임](https://seung-won-yu.github.io/rune-drift-survivors/survivor/)을 headless Chrome으로 확인했다. HTML의 모듈 경로, JavaScript/CSS/모듈 프리로드와 PNG 5개의 SHA-256이 로컬 Pages 빌드와 일치한다. 실제 시작·키보드 이동, 일시정지 중 시간/카메라 정지와 재개, 전문화 요약, 390×844 조이스틱 배치와 가로 넘침 부재를 확인했다. `?qa`에서도 개발용 QA 전역은 없고 페이지 예외와 HTTP 오류는 0개다. 저장·재도전과 이벤트 전체 흐름은 앞의 실제 시간 3판 및 원격 테스트에서 검증했다. [공개 확인 결과](./expansion-public-release-2026-09-09.json).

![공개 버전의 전문화·유물 요약](./images/expansion-public-pause.png)

이 전달 기록과 README 링크를 마무리하는 후속 커밋은 문서만 바꾸므로 `[skip ci]`로 기록한다. 공개 실행 코드는 위에서 전체 검사를 통과한 `88a1d38`과 동일하다. 물리 모바일/iOS Safari 성능과 사람의 재미 평가는 후속 검증 대상이다.
