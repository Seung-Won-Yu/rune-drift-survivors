...
The current lossless WebP sources are RGB with a pale neutral checker. Their decoded pixels are identical to the generated PNG sources while the combined deployable size is about 30% smaller. The shared `src/world/spriteAtlasTexture.js` removes that backdrop into a cached RGBA canvas before filtering. Fully transparent pixels are black, and the resulting texture uses trilinear mipmap filtering to reduce sparkle when characters appear small. Each renderer owns its texture and disposes it on unmount; the loaded source image is preserved. A future true-alpha export can remove this compatibility conversion without changing frame selection. See the [character and world pass](./design/gameplay-reframe/CHARACTER_AND_WORLD_PASS.md) for the current presentation and verification.
