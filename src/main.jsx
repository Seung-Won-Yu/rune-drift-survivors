...
function useInstancedModelParts(url, normalizeOrigin = false) {
...
function BalancedCasualArena({ visualQuality = 'balanced' }) {
  const arena = useMemo(() => {
    ...
    const trailSegments = laneAngles.flatMap((angle, laneIndex) => (
      Array.from({ length: visualQuality === 'low' ? 3 : 4 }, (_, index) => {
    ...
    const flowerCount = Math.round((visualQuality === 'low' ? 10 : 20) * density);
    ...
    const treeCount = visualQuality === 'low' ? 10 : 18;
    const trees = Array.from({ length: treeCount }, (_, index) => {
      ...
      const radius = 84 + (index % 4) * 7.8 + Math.cos(index * 0.9) * 2.6;
      const scale = 1.06 + (index % 4) * 0.12;
      return {
    ...
    const rockCount = visualQuality === 'low' ? 6 : 11;
    ...
    const grassCount = visualQuality === 'low' ? 12 : 22;
    ...
    const bushes = Array.from({ length: visualQuality === 'low' ? 10 : 18 }, (_, index) => {
    ...
    const edgeShadePatches = Array.from({ length: visualQuality === 'low' ? 5 : 8 }, (_, index) => {
      const angle = index * Math.PI * 2 / (visualQuality === 'low' ? 5 : 8) + 0.18;
  ...
  return (
    <group>
      <GroundDecalInstances transforms={arena.edgeShadePatches} shape="circle" segments={28} opacity={visualQuality === 'low' ? 0.055 : 0.075} />
      <GroundDecalInstances transforms={arena.centralPlaza} shape="circle" segments={40} opacity={visualQuality === 'low' ? 0.12 : 0.16} />
      <GroundDecalInstances transforms={arena.trailSegments} shape="plane" opacity={visualQuality === 'low' ? 0.07 : 0.1} doubleSide />
      <GroundDecalInstances transforms={arena.meadowPatches} shape="circle" segments={24} opacity={visualQuality === 'low' ? 0.035 : 0.052} />
      <GroundDecalInstances transforms={arena.shrinePads} shape="circle" segments={28} opacity={visualQuality === 'low' ? 0.06 : 0.082} />
      <GroundDecalInstances transforms={arena.pondPatches} shape="circle" segments={24} opacity={visualQuality === 'low' ? 0.075 : 0.105} />
      <GroundDecalInstances transforms={arena.pondHighlights} shape="ring" ringArgs={[0.42, 0.52, 18]} opacity={visualQuality === 'low' ? 0.075 : 0.11} doubleSide />
      <GroundDecalInstances transforms={arena.treeShadows} shape="circle" segments={18} opacity={visualQuality === 'low' ? 0.04 : 0.06} />
...
function BalancedNatureAssetAccents() {
...
function CasualFieldBackdrop({ visualQuality = 'high' }) {
  const field = useMemo(() => {
...
function SculptedRuinTerrain({ visualQuality = 'high' }) {
  const geometry = useMemo(() => {
    ...
    const colors = [];
    const indices = [];
    const lowColor = new THREE.Color('#5f7547');
    const midColor = new THREE.Color('#8eaa63');
    const highColor = new THREE.Color('#b9a667');
    ...
    const dryGrass = new THREE.Color('#bba35f');
    const pathDust = new THREE.Color('#b88955');
...
function RuneRelicLandmarks({ visualQuality = 'high' }) {
...
function RelicBoxInstances({ transforms, roughness = 0.94 }) {
  ...
  useEffect(() => {
    ...
    transforms.forEach((mark, index) => {
      local.pos.fromArray(mark.position);
      local.quat.setFromEuler(new THREE.Euler(mark.rotation[0], mark.rotation[1], mark.rotation[2]));
      local.scale.set(mark.scale[0], mark.scale[1], mark.scale[2]);
      local.matrix.compose(local.pos, local.quat, local.scale);
...
function RelicOctahedronInstances({ transforms, opacity = 0.42 }) {
  const meshRef = useRef();
  const local = useMemo(() => ({
    matrix: new THREE.Matrix4(),
    quat: new THREE.Quaternion(),
...
function GroundDecalInstances({
  ...
  useEffect(() => {
    if (!meshRef.current) return;
    transforms.forEach((mark, index) => {
      if (Array.isArray(mark.position)) local.pos.fromArray(mark.position);
      else local.pos.copy(mark.position);
      local.quat.setFromEuler(new THREE.Euler(-Math.PI / 2, 0, mark.rotation ?? 0));
      const scale = mark.scale ?? [1, 1, 1];
      if (Array.isArray(scale)) local.scale.set(scale[0], scale[1], scale[2] ?? 1);
      else local.scale.setScalar(scale);
      local.matrix.compose(local.pos, local.quat, local.scale);
      meshRef.current.setMatrixAt(index, local.matrix);
...
function PerimeterGroveSilhouettes({ visualQuality = 'high' }) {
  const transforms = useMemo(() => {
...
function FieldBiomeLandmarks({ visualQuality = 'high' }) {
  const marks = useMemo(() => {
...
function FieldPathNetwork({ visualQuality = 'high' }) {
  const marks = useMemo(() => {
...
function FieldCampLandmarks({ visualQuality = 'high' }) {
  const marks = useMemo(() => {
...
function OpenFieldTerrainIdentity({ visualQuality = 'high' }) {
...
function TerrainStoneInstances({ transforms, roll = 0.04, roughness = 0.94 }) {
  const stoneRef = useRef();
  const local = useMemo(() => ({
    matrix: new THREE.Matrix4(),
    quat: new THREE.Quaternion(),
  ...
  useEffect(() => {
    ...
    stoneRef.current.count = transforms.length;
...
function StaticModelInstances({ url, transforms, castShadow = false, receiveShadow = false, materialColor, normalizeOrigin = false }) {
