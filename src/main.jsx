...
import { IMPORTED_ENV_MODEL_URLS, MODEL_URLS, NATURE_MODEL_URLS, PRELOAD_MODEL_URLS, PROJECTILE_MODEL_URLS } from './config/assets.js';
import './styles.css';
...
function SculptedRuinTerrain({ visualQuality = 'high' }) {
  const geometry = useMemo(() => {
    ...
    const colors = [];
    const indices = [];
    const lowColor = new THREE.Color('#1f2f25');
    const midColor = new THREE.Color('#49603f');
    const highColor = new THREE.Color('#796d4a');
    const mossColor = new THREE.Color('#4c7b56');
    const edgeColor = new THREE.Color('#08110f');
    const warmStone = new THREE.Color('#716f61');
    const lowlandMud = new THREE.Color('#283428');
    const riftBlue = new THREE.Color('#1a6964');
    const dryGrass = new THREE.Color('#6d6742');
    const pathDust = new THREE.Color('#7c6c4d');
    const runeWash = new THREE.Color('#275f5d');
    const forestShade = new THREE.Color('#132016');
    ...
    for (let zIndex = 0; zIndex <= segments; zIndex += 1) {
      for (let xIndex = 0; xIndex <= segments; xIndex += 1) {
        ...
        color.lerp(forestShade, smoothStep(78.0, 112.0, radius) * 0.22);
  ...
  return (
    <mesh receiveShadow geometry={geometry}>
      <meshStandardMaterial vertexColors roughness={0.99} metalness={0.01} emissive="#19281d" emissiveIntensity={0.16} />
    </mesh>
  );
}
...
function ImportedForestGroundShadows({ transforms }) {
  ...
  return (
    <instancedMesh ref={shadowRef} args={[null, null, transforms.length]} frustumCulled={false}>
      ...
      <meshBasicMaterial color="#06100b" transparent opacity={0.22} depthWrite={false} toneMapped={false} />
    </instancedMesh>
  );
}
...
function ImportedForestLightFlecks({ transforms }) {
  ...
  useEffect(() => {
    ...
    transforms.forEach((transform, index) => {
      ...
      local.color.set(index % 4 === 0 ? ART_TOKENS.wornGold : ART_TOKENS.runeMint);
  ...
  return (
    <instancedMesh ref={fleckRef} args={[null, null, transforms.length]} frustumCulled={false}>
      <planeGeometry args={[1, 1]} />
      <meshBasicMaterial transparent opacity={0.24} depthWrite={false} side={THREE.DoubleSide} toneMapped={false} />
    </instancedMesh>
  );
}
